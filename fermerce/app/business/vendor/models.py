...
from fermerce.app.payment.models import TransferPayment
...
class Vendor(models.Model):
    ...
    payments: fields.ManyToManyRelation[TransferPayment] = fields.ManyToManyField(
        "models.TransferPayment", related_name="payouts", through="fm_vendor_payout"
    )
...
class VendorVerification(models.Model):
    ...
    identity_number_optional = fields.CharField(max_length=15, null=True)
    is_verified = fields.BooleanField(default=False)
    created_at = fields.DatetimeField(auto_now_add=True)
    modified_at = fields.DatetimeField(auto_now=True)
