...
async def verify_charges(
    ...
    if get_unfinished_payment:
        response = await services.charge_verification(
            get_unfinished_payment.reference
        )
        if response.status:
            ...
            if response.data.authorization:
                await models.SaveCard.create(
                    **response.data.authorization, user=user
                )
