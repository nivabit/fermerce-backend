...
from fermerce.app.market.cart.models import Cart
from fermerce.app.market.selling_units.models import ProductSellingUnit
from fermerce.app.users.account.models import User
from fermerce.core.enum.sort_type import SortOrder
from fermerce.core.services.base_old import filter_and_list
from fermerce.lib.exceptions import exceptions
from fermerce.app.market.cart import models, schemas
from fermerce.app.market.product.models import Product


async def create(data_in: schemas.ICartIn, user: User) -> models.Cart:
    get_product = await Product.query().get_or_none(id=data_in.product_id)
    if not get_product:
    ...
    if not new_cart:
        raise exceptions.ServerError("Error add product to cart")
    return new_cart
