...
async def create(
    ...
    try:
        ...
        if bad_types:
            raise exceptions.BadDataError("Unsupported media type, expected")
        to_out = []
        ...
        raise exceptions.ServerError("Error while creating resource" + str(e))
...
async def update(uri: str, media_obj: UploadFile) -> IResponseMessage:
    try:
        ...
        if not get_media:
            raise exceptions.NotFoundError(f"Resource with URI {uri} not found")
        bad_types = models.Media.check_file_type(
            file_type=models.Media.all_allowed_files, media_objs=[media_obj]
        )
        if bad_types:
            raise exceptions.BadDataError("Unsupported media type")
