...
from fermerce.app.message import models, schemas
...
async def create(data_in: schemas.IMessageIn, staff: Staff) -> models.Message:
    ...
    if new_message:
        return new_message
    raise exceptions.ServerError("Error while creating message")
...
async def update(
    ...
    if not get_message:
        raise exceptions.NotFoundError("message not found")
    if get_message:
        get_message.update_from_dict(message=data_in.message, sender=staff)
        await get_message.save()
        return get_message
    raise exceptions.ServerError("error updating message")
...
async def get(
    ...
    if not result:
        raise exceptions.BadDataError("message not found")
    return result
