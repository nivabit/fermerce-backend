...
from fermerce.app.reviews import schemas, models
...
async def update(
    ...
    if not check_review:
        raise exceptions.NotFoundError("Product does not exist")
    if check_review.edit_limit == 3:
        raise exceptions.ForbiddenError("You have reached the edit limit")
    ...
    try:
        ...
        raise exceptions.ServerError("error updating review")
...
async def delete(
    ...
    if not get_review:
        raise exceptions.NotFoundError("Review does not exist")
