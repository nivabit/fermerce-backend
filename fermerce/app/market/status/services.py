...
async def create(
    ...
    if check_status:
        raise exceptions.DuplicateError("status already exists")
...
async def get(
    ...
    if not perm:
        raise exceptions.NotFoundError("status not found")
    return perm
...
async def update(
    ...
    if not check_status:
        raise exceptions.NotFoundError("status does not exist")
