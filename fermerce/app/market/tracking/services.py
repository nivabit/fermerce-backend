...
from fermerce.app.tracking import models, schemas
...
async def update(
    ...
    if get_track:
        raise exceptions.DuplicateError("tracking already exist for this order item")
    result = await models.Tracking.filter(
        id=track_id, order_item=get_order_item
    ).first()
    if not result:
        raise exceptions.NotFoundError("Tracking is not found")
    if result:
        result.update_from_dict(**data_in)
        await result.save()
        return result
    raise exceptions.ServerError("error updating order item tracking")
...
async def get(
    ...
    if not result:
        raise exceptions.BadDataError("order item tracking not found")
    return result
...
async def delete(track_id: uuid.UUID) -> Response:
    ...
    if not get_track:
        raise exceptions.NotFoundError("order item Tracking not found")
