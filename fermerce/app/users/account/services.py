...
from fermerce.app.user import schemas, models
...
async def create(data_in=schemas.IUserIn):
    ...
    if check_user:
        raise exceptions.DuplicateError("Account already exist")
    ...
    raise exceptions.ServerError("Could not create account, please try again")
...
async def update_user_details(data_in=schemas.IUserUpdateIn):
    ...
    if data_in.email and check_user.email != data_in.email:
        check_existing_email = await models.User.get_or_none(
            ...
            if data_in.email
            else None | Q(username=data_in.username) if data_in.email else None
        )
        if str(check_existing_email.id) != check_user.id:
            raise exceptions.BadDataError("Account with this details already exists")
    users_update = check_user.update_from_dict(
        data_in.model_dump(exclude={"id"}, exclude_unset=True),
    )
    await users_update.save()

    if not users_update:
        raise exceptions.ServerError("Could not updating  details, please try again")
...
async def login(
    ...
    if not check_user.check_password(data_in.password):
        raise exceptions.UnauthorizedError(detail="incorrect email or password")
    if check_user.is_archived:
        raise exceptions.UnauthorizedError()
    if check_user.is_suspended:
        raise exceptions.UnauthorizedError("Your account is suspended")
    if not check_user.is_verified and check_user.is_active:
        await tasks.send_users_activation_email.kiq(
            dict(
                ...
                full_name=(
                    f"{check_user.firstname} {check_user.lastname}"
    ...
    if not token:
        raise exceptions.ServerError("Count not authenticate user")
    return token
...
async def verify_users_email(
    ...
    if not data.get("user_id", None):
        raise exceptions.BadDataError("Invalid token data")
    users_obj = await models.User.get_or_none(id=data.get("user_id", None))
    if not users_obj:
        raise exceptions.BadDataError("Invalid token")
    ...
    raise exceptions.ServerError("Could not activate account, please try again")
    ...
    if not users_obj.is_verified:
        await tasks.send_users_activation_email.kiq(
            user=dict(
                ...
                id=str(users_obj.id),
                full_name=(
                    f"{users_obj.firstname} {users_obj.lastname}"
...
async def update_user_password(
    ...
    raise exceptions.BadDataError("Invalid token was provided")
...
async def update_users_password_no_token(
    ...
    if not user_obj.check_password(data_in.old_password.get_secret_value()):
        raise exceptions.BadDataError("Old password is incorrect")

    if user_obj.check_password(data_in.password.get_secret_value()):
        raise exceptions.BadDataError("Try another password you have not used before")
...
async def get_user(user_id: uuid.UUID, load_related: bool = False) -> models.User:
    ...
    try:
        ...
        if not result:
            raise exceptions.NotFoundError("No user with the provided credential")
        if load_related:
            # return schemas.IUserFullOut(**result)
            return result
        return result
    except Exception as e:
        raise exceptions.ServerError(f"Error getting user data {e}")
