...
from fermerce.app.staff import schemas, models


async def create(
    data_in=schemas.IStaffIn,
):
    check_user = await User.get_or_none(id=data_in.user_id).select_related(
        "staff"
    )
    if not check_user:
        raise exceptions.NotFoundError("User not found")
    if check_user.staff:
        raise exceptions.DuplicateError("Staff already exist")
    new_Staff = await models.Staff.create(user=check_user)
    if new_Staff:
        return IResponseMessage(
            message="Staff account was created successfully"
        )
    raise exceptions.ServerError("Error staff account")
...
async def remove_staff_data(data_in: schemas.IRemoveStaff) -> None:
    ...
    if staff_to_remove:
        if data_in.permanent:
            ...
            await staff_to_remove.update_from_dict(
                dict(is_active=False, archived=True)
            )
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    raise exceptions.NotFoundError(
        f"Staff with staff_id {data_in.staff_id} does not exist"
    )
...
async def get_staff_details(user: User, load_related: bool = False):
    ...
    if not result:
        raise exceptions.NotFoundError("No staff with the provided credential")
    return result
...
async def get_staff(staff_id: uuid.UUID, load_related: bool = False):
    ...
    try:
        ...
        raise exceptions.ServerError(f"Error getting staff data {e}")
...
async def add_staff_permission(
    ...
    if not get_staff:
        raise exceptions.NotFoundError("Staff not found")
    get_perms = await Permission.filter(id__in=data_in.permissions).all()
    existed_perm = []
    if not get_perms:
        raise exceptions.NotFoundError("permissions not found")
...
async def remove_staff_permissions(
    ...
    if not check_perms:
        raise exceptions.NotFoundError(detail=" Permissions not found")
