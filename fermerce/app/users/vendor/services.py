...
async def create(data_in=schemas.IVendorIn):
    ...
    if check_business_name:
        raise exceptions.BadDataError("Business account already")
    to_create = dict(
        business_name=data_in.business_name,
    ...
    raise exceptions.ServerError("Error creating business account")
...
async def update(
    ...
    if not check_vendor:
        raise exceptions.NotFoundError("Business Not found")
    check_business_name = await models.Vendor.get_or_none(
        business_name=data_in.business_name
    )
...
async def login_vendor(
    ...
    if not check_vendor:
        raise exceptions.UnauthorizedError(detail="incorrect email, username or password")
    ...
    if not token:
        raise exceptions.ServerError("Count not authenticate vendor account")
    return token
...
async def get_vendor_details(vendor_id: uuid.UUID, load_related: bool = False):
    ...
    if result is not None:
        return result
    raise exceptions.NotFoundError("Vendor not found")
