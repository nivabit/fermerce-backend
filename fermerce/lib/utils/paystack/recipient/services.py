import typing as t
from fermerce.lib.utils.paystack.recipient import schemas
...
async def create_bulk_trans_recipient(data_in: t.List[schemas.ITransactionRecipientIn]):
    try:
        response = await client.request.post(
            url=endpoint.get("transfers_recipient").get("create"), json=data_in.dict()
        )
...
async def list_trans_recipient():
    try:
        response = await client.request.post(
